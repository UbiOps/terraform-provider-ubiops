...
func NewUbiOpsClient(baseURL, apiToken, providerVersion, resolveIP string) *UbiOpsClient {
	// UbiOps's dashboard hands out tokens pre-fixed with "Token " for direct copy-paste into an
	// Authorization header. Strip it here so callers pasting that verbatim don't end up with a
	// doubled "Token Token <token>" header (rejected by the API as "should not contain spaces").
