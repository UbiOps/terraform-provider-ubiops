...
func TestAccServiceResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccCheckDestroyed("ubiops_service", func(a map[string]string) string {
			return fmt.Sprintf("/projects/%s/services/%s", a["project_name"], a["name"])
		}),
		Steps: []resource.TestStep{
			...
			// Create service; promotes "v1" to default_version now that it exists.
			{
...
func testAccServiceDeploymentOnlyConfig(projectName, deploymentName string) string {
	return fmt.Sprintf(`
