...
func TestAccRequestScheduleResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// before it exists.
			{
				Config: testAccRequestScheduleDeploymentOnlyConfig(projectName, deploymentName),
			},
			// Create schedule; promotes "v1" to default_version now that it exists.
			{
...
func testAccRequestScheduleDeploymentOnlyConfig(projectName, deploymentName string) string {
	return fmt.Sprintf(`
...
resource "ubiops_request_schedule" "test" {
  ...
  schedule          = %[4]q
  enabled           = false
  request_data_json = jsonencode({ input = "test-value" })
