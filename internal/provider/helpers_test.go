...
// testAccResourceName builds a lowercase, UbiOps-safe resource name from the test's name.
...
func testAccFirstInstanceTypeID(t *testing.T, projectName string) string {
	...
	if err := c.Get(context.Background(), fmt.Sprintf("/projects/%s/instance-types", projectName), &instanceTypes); err != nil {
		t.Fatalf("failed to list instance types: %v", err)
	}
	if len(instanceTypes) == 0 {
		t.Fatal("project has no available instance types")
	}
	return instanceTypes[0].ID
}
