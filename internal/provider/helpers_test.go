...
// testAccResourceName builds a lowercase, UbiOps-safe, unique resource name from the
// test's name plus a random suffix. A random suffix is required, not just cosmetic:
// CI runs the 1.13.*/1.14.* Terraform version matrix jobs concurrently against the same
// shared project, and this suite may also run repeatedly from a local dev loop against
// that same project - a name derived purely from t.Name() collides across those runs.
// The test name is truncated to leave room for both the suffix and further caller-added
// suffixes (e.g. "-dep", "-updated") within UbiOps's 64-character name limit.
