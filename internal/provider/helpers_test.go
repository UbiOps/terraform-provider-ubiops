...
// testAccResourceName builds a UbiOps-safe acceptance-test resource name from the
// current test's name. UbiOps object names only allow lowercase letters, digits, and
// dashes; Go test names are PascalCase (e.g. "TestAccDeploymentResource"), so this
// lowercases it to avoid "name is not allowed" API errors.
func testAccResourceName(t *testing.T) string {
	t.Helper()
	return fmt.Sprintf("tf-acc-test-%s", strings.ToLower(t.Name()))
}
